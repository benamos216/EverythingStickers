...
$pp_methods = array(
		...
		array(
			'title' => esc_html_x( 'Payment methods', 'Title of setting tab', 'yith-paypal-payments-for-woocommerce' ),
			'type'  => 'title',
			'desc'  => '',
			'id'    => 'yith_ppwc_payment_methods_options',
		),
		...
		array(
			...
			'desc'      => sprintf( esc_html_x( 'Select the additional payments buttons that will be available in the PayPal wallet by default.%sWhether these are eligible and visible to the user, this will depend on a variety of factors including customer location.',
				'Admin option. Placeholder is an html tag.', 'yith-paypal-payments-for-woocommerce' ), '<br>' ),
			'type'      => 'yith-field',
		...
		array(
			'type' => 'sectionend',
			'id'   => 'yith_ppwc_end_payment_methods_options',
		),
);
...
$cc_cart = array(
	...
	array(
		...
		'desc'              => esc_html_x( '3D Secure enables you to authenticate card holders through card issuers. It reduces the likelihood of fraud when you use supported cards and improves transaction performance. A successful 3D Secure authentication can shift liability for chargebacks due to fraud from you to the card issuer.',
			'Admin description option', 'yith-paypal-payments-for-woocommerce' ),
		'type'              => 'yith-field',
		'yith-type'         => 'onoff',
		'default'           => 'yes',
		'custom_attributes' => array(
...
$remove_cc_settings = apply_filters( 'yith_paypal_payments_remove_cc_settings', false );
return array( 'payment-methods' =>$remove_cc_settings ? $pp_methods : array_merge( $pp_methods, $cc_cart ));